...
- **Gate discrimination — honest characterization of the real embedder (round-2 C1 / round-3 I4)** — with the actual `HashingEmbedder` (which does **NOT** strip stopwords — `text.lower().split()`, so shared `the/to/a/how` inflate cosine), two characterization tests pin the *true* behaviour rather than an idealized one: **(a)** a **lexically-disjoint** off-topic chunk (no shared tokens → cos ≈ 0) is excluded while a topically-overlapping chunk clears `0.15` — so the gate reliably rejects disjoint content; **(b)** a **stopword-only-overlapping** chunk (shares only `how/the/and` with the prompt, no topical tokens) **scores well above `0.15` and is NOT excluded** — empirically, with ~5 shared stopwords cosine lands ≈ 0.7. Test (b) deliberately asserts the *false positive* to document that the `0.15` gate is **lexical-and-permissive, not semantic**: stopword overlap defeats it. This is the known M4a limitation — the placeholder threshold + bge semantic gate are M4b's job; a zero-shared-token semantic match is the other failure mode (also not found until bge).
