...
def _gold_third(fx: Fixture) -> int:
    ...
    positions = [keys.index(g) for g in fx.gold_keys if g in keys]
    if not positions:                      # gold absent from chunks -> anomalous; treat as oldest
        return 0
    frac = min(positions) / max(1, len(keys) - 1)
